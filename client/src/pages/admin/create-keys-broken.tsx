...
export default function AdminCreateKeys() {
  ...
  return (
    <AdminLayout>
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "generate" | "manage")} className="w-full">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <TabsList className="grid w-full sm:w-auto grid-cols-2 bg-purple-900/30 border border-purple-500/30">
            <TabsTrigger 
              value="generate" 
              className="data-[state=active]:bg-purple-600 data-[state=active]:text-white text-purple-300 transition-all duration-200"
                      ...
                      <div className=\"flex flex-col lg:flex-row lg:items-center justify-between gap-4\">
                        ...
                        <div className=\"flex flex-col sm:flex-row gap-2\">
                          <Dialog>
                            ...
                            <DialogContent className=\"max-w-2xl bg-gradient-to-br from-gray-900 to-gray-800 border border-purple-500/30\">
                              <DialogHeader>
                                <DialogTitle className=\"text-purple-300\">Key Details</DialogTitle>
                                <DialogDescription className=\"text-muted-foreground\">
                                  Detailed information about this license key
                                </DialogDescription>
                              </DialogHeader>
                              <div className=\"space-y-4\">
                                <div className=\"grid grid-cols-1 md:grid-cols-2 gap-4\">
                                  <div>
                                    <label className=\"text-sm font-medium text-purple-300\">Key String</label>
                                    <code className=\"block w-full p-2 bg-purple-900/30 text-purple-100 rounded border border-purple-500/30 font-mono text-sm break-all\">
                                      {key.keyString}
                                    </code>
                                  </div>
                                  <div>
                                    <label className=\"text-sm font-medium text-purple-300\">Status</label>
                                    <div className=\"mt-1\">{getStatusBadge(key.status)}</div>
                                  </div>
                                  <div>
                                    <label className=\"text-sm font-medium text-purple-300\">Game</label>
                                    <p className=\"text-muted-foreground\">{key.game}</p>
                                  </div>
                                  <div>
                                    <label className=\"text-sm font-medium text-purple-300\">Device Usage</label>
                                    <div className=\"mt-1\">
                                      <div className=\"flex items-center justify-between text-sm\">
                                        <span>{key.deviceCount}/{key.deviceLimit} devices</span>
                                        <span>{Math.round((key.deviceCount / key.deviceLimit) * 100)}%</span>
                                      </div>
                                      <Progress value={(key.deviceCount / key.deviceLimit) * 100} className=\"mt-1\" />
                                    </div>
                                  </div>
                                  <div>
                                    <label className=\"text-sm font-medium text-purple-300\">Created At</label>
                                    <p className=\"text-muted-foreground\">{format(new Date(key.createdAt), 'MMM dd, yyyy HH:mm')}</p>
                                  </div>
                                  <div>
                                    <label className=\"text-sm font-medium text-purple-300\">Expiry Date</label>
                                    <p className=\"text-muted-foreground\">{format(new Date(key.expiryDate), 'MMM dd, yyyy HH:mm')}</p>
                                  </div>
                                </div>
                                {key.devices && key.devices.length > 0 && (
                                  <div>
                                    <label className=\"text-sm font-medium text-purple-300 mb-2 block\">Connected Devices</label>
                                    <div className=\"space-y-2 max-h-32 overflow-y-auto\">
                                      {key.devices.map((device: any) => (
                                        <div key={device.id} className=\"flex items-center justify-between p-2 bg-purple-900/20 rounded border border-purple-500/20\">
                                          <code className=\"font-mono text-xs text-purple-200\">{device.deviceId}</code>
                                          <span className=\"text-xs text-muted-foreground\">{format(new Date(device.firstConnected), 'MMM dd, HH:mm')}</span>
                                        </div>
                                      ))}
                                    </div>
                                  </div>
